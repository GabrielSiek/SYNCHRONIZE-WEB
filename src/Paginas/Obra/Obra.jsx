...
const Obra = () => {
  ...
  const postRow = async (row, updateRow) => {
    api
      .put(`/item/${row.id}/update`, updateRow)
      .then((response) => {
  ...
  const getRow = async (row) => {
    try {
      return (newRow = await api.get(`/item/${row.id}/info`));
    } catch (e) {
      if (e.response) {
  ...
  const handleCellChange = useCallback(
    (newRow, oldRow) => {
      ...
      const isDifferent = updatedItens.some((item, index) => {
        ...
        return (
          item.preparacao_desenvolvimento_area !==
            obraItem.preparacao_desenvolvimento_area ||
          item.preparacao_desenvolvimento_porcentagem !==
            obraItem.preparacao_desenvolvimento_porcentagem ||
          item.protecao_desenvolvimento_area !==
            obraItem.protecao_desenvolvimento_area ||
          item.protecao_desenvolvimento_porcentagem !==
            obraItem.protecao_desenvolvimento_porcentagem ||
          item.desenvolvimento_area !== obraItem.desenvolvimento_area ||
          item.desenvolvimento_porcentagem !==
            obraItem.desenvolvimento_porcentagem
        );
      });
  ...
  const handleChangeTab = (event, newTab) => {
    ...
    if (newTab === 0) setItensTabela(obra.itens);
    else if (newTab === 1)
      setItensTabela(
        obra.itens.filter((item) => item.status === "NAO_CONCLUIDO")
      );
    else if (newTab === 2)
      ...
      <div className="obra-conteudo" id="obra-conteudo">
        <div className="tabela-header">
          <div className="tabela-informacoes">
            ...
            <a
              className="tabela-header-link"
              href={`/encarregado/${obra.encarregado_id}`}
            >
              <FaHelmetSafety className="icon" />
              {obra.encarregado_nome}
            </a>
            <a
              className="tabela-header-link"
              href={`/obra/${obra_id}/diario-de-obra`}
            >
              <PiBookBold /> Diário de obra
            </a>
          </div>
          <Header />
        </div>
